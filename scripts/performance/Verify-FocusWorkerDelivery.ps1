...
function Read-Json([string]$Path) { [void](Pin $Path); Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json -DateKind String }
...
$reverificationPath = Join-Path $validationRoot 'focus-worker-measurement-reverification.json'
...
Require ($CandidateSnapshotId -ceq 'FWM-FOCUS-WORKER-ADMISSION-20260909-C1' -and
    $BaselineSnapshotId -ceq 'FWM-FOCUS-WORKER-ADMISSION-20260909-R0' -and
    $ExperimentId -ceq 'EXP-FOCUS-030-WORKER-ADMISSION-C1') 'Delivery must verify the accepted FocusWorkerAdmission R0/C1 experiment.'
$pins = [ordered]@{}
...
$r0 = Read-Snapshot $BaselineSnapshotId
$c1 = Read-Snapshot $CandidateSnapshotId
$c2 = Read-Snapshot $SnapshotId
Require ($r0.Map.Count -eq $c1.Map.Count) 'R0/C1 source sets differ.'
$baselineDifferences = @($c1.Map.Keys | Where-Object { !$r0.Map.ContainsKey($_) -or $r0.Map[$_] -cne $c1.Map[$_] } | Sort-Object)
Require (($baselineDifferences -join '|') -ceq 'FancyWM/Utilities/FocusHelper.cs') 'R0/C1 difference exceeds the lazy bound ThreadStart reuse in FocusHelper.RequestSequence.Enqueue.'
$deliveryAllowed = @('PERFORMANCE_STATUS.md', 'PERFORMANCE_TODO.md', 'docs/performance/AUDIT.md',
    ...
    'scripts/performance/README.md', 'scripts/performance/Verify-FocusWorkerDelivery.ps1')
...
Require ($c2.Map['scripts/performance/Verify-FocusWorkerDelivery.ps1'] -ceq $writerHash) 'Delivery verifier is not the frozen C2 source.'
$functionalPins = @{
...
foreach ($relative in $functionalPins.Keys) {
    ...
    foreach ($snapshot in @($r0, $c1, $c2)) { Require ($snapshot.Map[$relative] -ceq $functionalPins[$relative]) "Functional document changed: $relative" }
}

$strictPath = Join-Path $candidateRoot 'validation/focus-worker-strict-verification.json'
[void](Pin $strictPath '4EAC61B5DCE99BA848563328E1DFEB118D8AE653D867B92F8958F1380DD4054B')
$strict = Read-Json $strictPath
Require ($strict.Verdict -ceq 'ACCEPT' -and $strict.Accepted -is [bool] -and $strict.Accepted -and
    $strict.Scenario -ceq 'FocusWorkerAdmission' -and $strict.PerfId -ceq 'PERF-030' -and
    ...
    $strict.MeasurementRows -eq 570 -and $strict.RawCounters -eq 570 -and $strict.ProcessCount -eq 10 -and
    $strict.AllocationPairs -eq 15 -and $strict.Allocation.Count -eq 15 -and $strict.PairedSemanticComparisons -eq 240 -and
    $strict.SemanticMetricCount -eq 16 -and $strict.FrequencyInvariantComparisons -eq 29 -and
    $strict.CandidatePreparedRegressionLeaves -eq 8 -and $strict.CandidatePreparedRegressionPassed -eq 8 -and
    $strict.BinaryTreeFiles -eq 198 -and $strict.BinaryCheckRows -eq 1980 -and $strict.ProcessTimesRows -eq 10 -and
    $strict.BaselineRegressionLeaves -eq 8 -and $strict.BaselineSemanticTestsPassed -eq 5 -and
    $strict.BaselineWorkerAdmissionAllocationTestsFailed -eq 3 -and
    $strict.TestFqn -ceq 'FancyWM.Tests.Utilities.FocusHelperTest.FocusWorkerAdmissionCounterScenario' -and
    ($strict.SourceManifestDifferences -join '|') -ceq 'FancyWM/Utilities/FocusHelper.cs' -and
    ($strict.BinaryDifferences -join '|') -ceq 'FancyWM.dll' -and
    ($strict.ProcessOrder -join '|') -ceq 'baseline-1|candidate-1|candidate-2|baseline-2|baseline-3|candidate-3|candidate-4|baseline-4|baseline-5|candidate-5' -and
    $strict.TimingClassification -cin @('all measured pairs lower','all measured pairs higher','mixed') -and
    $strict.PairedTimingWins -ge 0 -and $strict.PairedTimingLosses -ge 0 -and $strict.PairedTimingTies -ge 0 -and
    $strict.PairedTimingWins + $strict.PairedTimingLosses + $strict.PairedTimingTies -eq 15 -and
    $strict.TimingAcceptanceGate -is [bool] -and !$strict.TimingAcceptanceGate -and
    $strict.BaselineManifestSHA256 -ceq $r0.ManifestSHA256 -and $strict.CandidateManifestSHA256 -ceq $c1.ManifestSHA256 -and
    $strict.BaselineSnapshotSHA256 -ceq $r0.SnapshotSHA256 -and $strict.CandidateSnapshotSHA256 -ceq $c1.SnapshotSHA256) 'C1 strict acceptance differs.'
$allocationKeys = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($allocation in $strict.Allocation) {
    Require ($allocation.Case -cin @('direct-success','alt-retry','attach-failure') -and
        ...
        $allocation.SavedBytes -eq 32000 -and $allocation.BaselineBytesPerCall -eq 368 -and
        $allocation.CandidateBytesPerCall -eq 304 -and $allocation.SavedBytesPerCall -eq 64 -and
        $allocation.SavedBytes -eq $allocation.BaselineBytes - $allocation.CandidateBytes) 'Accepted allocation pair differs.'
}
Require ($allocationKeys.Count -eq 15) 'Accepted allocation matrix is incomplete.'

foreach ($entry in @(@('measurements.csv', 'MeasurementsSHA256'), @('binary-checks.csv', 'BinaryChecksSHA256'), @('process-times.csv', 'ProcessTimesSHA256'))) {
    [void](Pin (Join-Path $experimentRoot $entry[0]) $strict.($entry[1]))
}
$measurementProvenancePath = Join-Path $experimentRoot 'measurement-provenance.json'
[void](Pin $measurementProvenancePath '8339CC7739648EE5D30BCD3B4167417C94FFFB6ED78530C1509BA638662F4BB2')
$measurementProvenance = Read-Json $measurementProvenancePath
Require ($measurementProvenance.ExperimentId -ceq $ExperimentId -and
    $measurementProvenance.BaselineSnapshotId -ceq $BaselineSnapshotId -and
    $measurementProvenance.CandidateSnapshotId -ceq $CandidateSnapshotId -and
    $measurementProvenance.Decision -ceq 'MEASUREMENT_CHECKS_PASS' -and
    $measurementProvenance.MeasurementRows -eq 570 -and $measurementProvenance.ProcessCount -eq 10 -and
    $measurementProvenance.SemanticComparisonsMatched -eq 240 -and $measurementProvenance.AllocationPairsImproved -eq 15 -and
    $measurementProvenance.TimingWins -eq $strict.PairedTimingWins -and
    $measurementProvenance.TimingLosses -eq $strict.PairedTimingLosses -and
    $measurementProvenance.TimingTies -eq $strict.PairedTimingTies -and
    $measurementProvenance.TimingClassification -ceq 'mixed') 'Runner measurement provenance differs.'
$measurementEvidencePaths = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($entry in $measurementProvenance.Evidence) {
    Require ($entry.Path -cin @('measurements.csv', 'raw-counters.csv', 'binary-checks.csv', 'process-times.csv', 'paired-results.csv', 'dotnet-info.log') -and
        $measurementEvidencePaths.Add($entry.Path)) 'Unexpected or duplicate runner evidence path.'
    [void](Pin (Safe-Child $experimentRoot $entry.Path) $entry.SHA256)
}
Require ($measurementEvidencePaths.Count -eq 6) 'Runner evidence set is incomplete.'
...
[void](Pin (Join-Path $candidateRoot 'release-test-binaries.csv') $strict.CandidateArchiveManifestSHA256)
[void](Pin (Join-Path $candidateRoot 'release-test-binaries.provenance.json') $strict.CandidateProvenanceSHA256)
$measurementVerifier = Join-Path $c2.Root 'source/scripts/performance/Verify-FocusWorkerAdmission.ps1'
...
foreach ($process in $strict.Processes) {
    ...
    [void](Pin (Join-Path $experimentRoot "$($process.Run).stdout.log") $process.StdOutSHA256)
    [void](Pin (Join-Path $experimentRoot "$($process.Run).stderr.log") $process.StdErrSHA256)
    [void](Pin (Join-Path $experimentRoot "$($process.Run).command.json") $process.CommandSHA256)
    [void](Pin (Join-Path $experimentRoot "$($process.Run).process.json") $process.ProcessSHA256)
    [void](Pin (Join-Path $experimentRoot "runs/$($process.Run)/binaries.csv") $process.BinaryManifestSHA256)
}
$appendPath = Join-Path $experimentRoot 'ledger-append.json'
[void](Pin $appendPath 'AC36AC9F843624C28E5780C018F70FA5EBE8A1BCEC663AF7CD794C58BFFF2B1D')
$append = Read-Json $appendPath
$prefixLength = 39228823L
$prefixHash = '407C85168F01D049B885E58255E10CCF98BE91A5DFA0C1F798080CAA77D2AE94'
Require ($append.Verdict -ceq 'ACCEPT' -and $append.PerfId -ceq 'PERF-030' -and $append.SnapshotId -ceq $CandidateSnapshotId -and $append.ExperimentId -ceq $ExperimentId -and
    ...
    $append.OldSHA256 -ceq $prefixHash -and $append.PrefixSHA256 -ceq $prefixHash -and $append.OldRows -eq 38160 -and $append.OldLines -eq 38161 -and
    $append.AppendedRows -eq 570 -and $append.NewRows -eq 38730 -and $append.NewLines -eq 38731 -and
    $append.NewLength -eq 39811448 -and $append.NewLength -eq $prefixLength + $append.SuffixLength -and
    $append.NewSHA256 -ceq '9D40BD0AE06DE654DBE0C9E0CE036E55B0463A892621DCA23A873CA3D3D1B0B1' -and
    $append.SuffixLength -eq 582625 -and $append.SuffixSHA256 -ceq '0ACFEC5424001D0236BF686FE9AE62A3DE38CC93CABF160F68EC70478E3414F0' -and
...
[void](Pin (Join-Path $repositoryRoot 'scripts/performance/Append-FocusWorkerMeasurements.ps1') $append.AppenderSHA256)
$ledgerPath = Join-Path $repositoryRoot $append.LedgerPath
[void](Pin $ledgerPath $append.NewSHA256)
Require ($c2.Map[$append.LedgerPath] -ceq $append.NewSHA256 -and $r0.Map[$append.LedgerPath] -ceq $prefixHash -and $c1.Map[$append.LedgerPath] -ceq $prefixHash) 'Frozen ledger identities differ.'
...
Require ([regex]::Matches($ledgerText, '\n').Count -eq 38731 -and
    [regex]::Matches($ledgerText, '(?m)^"' + [regex]::Escape($ExperimentId) + '",').Count -eq 570) 'Ledger row counts differ.'
...
$tests = @(foreach ($configuration in @('Debug', 'Release')) {
    foreach ($project in @('FancyWM.Tests', 'FancyWM.Layouts.Tests', 'FancyWM.ThemeEngine.Tests')) {
        $expected = switch ($project) { 'FancyWM.Tests' { if ($configuration -ceq 'Debug') { 2004 } else { 2060 } } 'FancyWM.Layouts.Tests' { 160 } 'FancyWM.ThemeEngine.Tests' { 31 } }
        $expectedRaw = switch ($project) { 'FancyWM.Tests' { if ($configuration -ceq 'Debug') { 2256 } else { 2323 } } 'FancyWM.Layouts.Tests' { 182 } 'FancyWM.ThemeEngine.Tests' { 31 } }
        $result = Read-PassedTrx (Join-Path $validationRoot "$configuration-$project.trx") $expected $validationRun
        Require ($result.Raw -eq $expectedRaw) "Full suite raw-result count differs: $configuration/$project"
        $result
    }
})
$priorRoot = Join-Path $performanceRoot 'FWM-FOCUS-REJECTED-ADMISSION-20260909-C2R1'
$priorSummaryPath = Join-Path $priorRoot 'validation/validation-summary.json'
[void](Pin $priorSummaryPath '4C86FD85DA66B14FBC4D48A6E81D66BD0D69D402D9670BDC556D5E69FD2A7A6B')
$prior = Read-Json $priorSummaryPath
Require ($prior.Verdict -ceq 'ACCEPT') 'Prior accepted gate changed.'
$newLeaves = @(
    'RepeatedCompletedWorkerAdmissionsPreserveSemantics (direct-success)',
    'RepeatedCompletedWorkerAdmissionsPreserveSemantics (alt-retry)',
    'RepeatedCompletedWorkerAdmissionsPreserveSemantics (attach-failure)',
    'RepeatedCompletedWorkerAdmissionsStayWithinCallerAllocationBudget (direct-success)',
    'RepeatedCompletedWorkerAdmissionsStayWithinCallerAllocationBudget (alt-retry)',
    'RepeatedCompletedWorkerAdmissionsStayWithinCallerAllocationBudget (attach-failure)',
    'CompletedWorkerStartDelegateDoesNotRootRequestSequence',
    'FocusWorkerAdmissionCounterScenario'
)
$affectedFragments = @('FocusHelperTest', 'MainWindowLifetimeTest', 'ModifierWindowMoverTest')
$affectedFilter = ($affectedFragments | ForEach-Object { "FullyQualifiedName~$_" }) -join '|'
$targetedMethods = @('RepeatedCompletedWorkerAdmissionsPreserveSemantics', 'RepeatedCompletedWorkerAdmissionsStayWithinCallerAllocationBudget', 'CompletedWorkerStartDelegateDoesNotRootRequestSequence', 'FocusWorkerAdmissionCounterScenario')
$targetedFilter = ($targetedMethods | ForEach-Object { "FullyQualifiedName=FancyWM.Tests.Utilities.FocusHelperTest.$_" }) -join '|'

$newKeysByConfiguration = @{}
$candidateTests = @(foreach ($configuration in @('Debug', 'Release')) {
    $targeted = Read-PassedTrx (Join-Path $candidateRoot "validation/$configuration-focus-worker-targeted.trx") 8
    $acceptedRegression = @($strict.CandidateRegressions | Where-Object Configuration -CEQ $configuration)
    Require ($acceptedRegression.Count -eq 1 -and $targeted.Raw -eq 10 -and
        $targeted.TrxSHA256 -ceq $acceptedRegression[0].TrxSHA256 -and $targeted.LogSHA256 -ceq $acceptedRegression[0].LogSHA256 -and
        (@($targeted.Names | Sort-Object) -join '|') -ceq (@($newLeaves | Sort-Object) -join '|')) "Targeted test composition differs: $configuration"
    $newKeysByConfiguration[$configuration] = $targeted.Keys
    $targeted
    $previousSuite = @($prior.Tests | Where-Object {
        [IO.Path]::GetFileName($_.Path) -ceq "$configuration-FancyWM.Tests.trx"
    })
    Require ($previousSuite.Count -eq 1) "Prior affected suite is missing: $configuration"
    $previousSuitePath = Safe-Child $repositoryRoot $previousSuite[0].Path
    [void](Pin $previousSuitePath $previousSuite[0].TrxSHA256)
    $previousTrx = Read-Xml $previousSuitePath
    $affectedIds = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($definition in @($previousTrx.SelectNodes('/*[local-name()="TestRun"]/*[local-name()="TestDefinitions"]/*[local-name()="UnitTest"]'))) {
        $methods = @($definition.SelectNodes('./*[local-name()="TestMethod"]'))
        Require ($methods.Count -eq 1) 'Prior test definition has ambiguous methods.'
        $fqn = $methods[0].className + '.' + $methods[0].name
        $matchesAffected = @($affectedFragments | Where-Object {
            $fqn.IndexOf($_, [StringComparison]::Ordinal) -ge 0
        }).Count -gt 0
        if ($matchesAffected) { Require ($affectedIds.Add($definition.id)) 'Prior affected test definition is duplicated.' }
    }
    Require ($affectedIds.Count -gt 0) 'Prior affected test selection is empty.'
    $previousAffectedResults = @($previousTrx.SelectNodes('//*[local-name()="UnitTestResult"]') | Where-Object {
        $affectedIds.Contains($_.testId)
    })
    $previousAffectedLeaves = @($previousAffectedResults | Where-Object {
        $_.SelectNodes('./*[local-name()="InnerResults"]/*[local-name()="UnitTestResult"]').Count -eq 0
    })
    $expectedAffectedKeys = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($leaf in $previousAffectedLeaves) {
        Require ($leaf.outcome -ceq 'Passed' -and $expectedAffectedKeys.Add($leaf.testId + '|' + $leaf.testName)) 'Prior affected leaf is failed or duplicated.'
    }
    foreach ($key in $targeted.Keys) { Require ($expectedAffectedKeys.Add($key)) 'A newly added targeted leaf already exists in the prior affected suite.' }
    $affected = Read-PassedTrx (Join-Path $candidateRoot "validation/$configuration-focus-worker-affected.trx") $expectedAffectedKeys.Count
    $affectedKeys = [string[]]@($affected.Keys)
    Require ($affected.Raw -eq 271 -and $affected.Leaves -eq 239 -and
        $affected.Raw -eq $previousAffectedResults.Count + $targeted.Raw -and
        $expectedAffectedKeys.SetEquals($affectedKeys)) "Affected test composition differs from the declared prior union plus eight new leaves: $configuration"
...
$composition = @(foreach ($test in $tests) {
    ...
    $expectedAdded = if ($leafName -like '*-FancyWM.Tests.trx') { @($newLeaves | Sort-Object) } else { @() }
    Require (($added -join '|') -ceq ($expectedAdded -join '|')) "Full test additions differ: $leafName"
    $configuration = if ($leafName.StartsWith('Debug-', [StringComparison]::Ordinal)) { 'Debug' } else { 'Release' }
    $addedKeys = @($differences | Where-Object SideIndicator -CEQ '=>' | ForEach-Object InputObject | Sort-Object)
    $expectedAddedKeys = if ($leafName -like '*-FancyWM.Tests.trx') { @($newKeysByConfiguration[$configuration] | Sort-Object) } else { @() }
    Require (($addedKeys -join '|') -ceq ($expectedAddedKeys -join '|')) "Full test additions have different identities from targeted C1: $leafName"
...
$expectedReleasePackageOutputs = @($releasePackageOutputs | Where-Object {
    [IO.Path]::IsPathRooted($_.Groups['path'].Value.Trim()) -and
    [StringComparer]::OrdinalIgnoreCase.Equals(
        [IO.Path]::GetFullPath($_.Groups['path'].Value.Trim()),
...
Require ($sourceArtifactHash -ceq $provenance.ArtifactSHA256 -and
    ...
    $releasePackageOutputs.Count -ge 1 -and $expectedReleasePackageOutputs.Count -eq 1) `
    'Release package source is not the exact output logged by this gate.'
...
$null = & $measurementVerifier -CandidateSnapshotId $CandidateSnapshotId -BaselineSnapshotId $BaselineSnapshotId -ExperimentId $ExperimentId -ReportPath $reverificationPath
...
$summary = [ordered]@{
    Verdict = 'ACCEPT'; ReportVersion = 1; SnapshotId = $SnapshotId; PerfId = 'PERF-030'
    BoundedStage = 'FocusHelper.RequestSequence.Enqueue reuses its lazy bound ThreadStart across completed worker lifetimes'
    Commit = $head; Tree = $tree; SnapshotManifestSHA256 = $c2.ManifestSHA256; SnapshotJSONSHA256 = $c2.SnapshotSHA256
    SnapshotSourceFiles = $c2.Files; AllFrozenSourceFilesVerified = $r0.Files + $c1.Files + $c2.Files; BuildInputsVerified = $buildInputs.Count
    BaselineSnapshot = $BaselineSnapshotId; CandidateSnapshot = $CandidateSnapshotId
    BaselineManifestSHA256 = $r0.ManifestSHA256; CandidateManifestSHA256 = $c1.ManifestSHA256
    ...
    C1MeasurementAcceptance = [ordered]@{
        ...
        ProcessCount = 10; MeasurementRows = 570; RawCounters = 570; AllocationPairs = 15; SemanticComparisons = 240
        BaselineRegressionLeaves = 8; BaselineSemanticTestsPassed = 5; BaselineWorkerAdmissionAllocationTestsFailed = 3
        TestFqn = $strict.TestFqn; CommonFixtureSHA256 = $strict.CommonFixtureSHA256
    ...
    TargetedFilter = $targetedFilter; AffectedFilter = $affectedFilter
    TestCompositionAgainstAcceptedPreviousGate = $composition
    OptimizationLedger = [ordered]@{
        Rows = 38730; Bytes = $ledger.Length; SHA256 = Hash $ledgerPath; PreviousRows = 38160; PreviousBytes = $prefixLength; PreviousSHA256 = $prefixHash
        AppendedRows = 570; AppendedBytes = $suffixLength; AppendedSHA256 = $append.SuffixSHA256; ExactHistoricalPrefix = $true; ExactAcceptedSuffix = $true; AppendProofSHA256 = Hash $appendPath
    }
    ...
    FunctionalDocuments = [ordered]@{ SHA256 = $functionalPins; PreservedAcrossR0_C1_C2_AndLive = $true }
    StatusCounts = [ordered]@{ IMPLEMENTED = 23; IN_PROGRESS = 14; WholeIdBlocked = 0 }
    Limitations = 'Managed caller allocation for repeated sequential FocusHelper.RequestSequence.Enqueue admissions after warmup, with real background threads joined by the fixture and fake native adapters. Lazy ThreadStart reuse saves 64 B per subsequent completed worker lifetime. The first lazy allocation, cross-thread allocations, production contention/scheduling, native focus/input/COM/WPF/DPI, timeout or late-focus effects, WPR, process CPU/GPU, presentation latency and energy remain E2E_PENDING / NOT_MEASURED. Timing includes start/join and is mixed and descriptive. PERF-030 and PERF-010 remain IN_PROGRESS.'
