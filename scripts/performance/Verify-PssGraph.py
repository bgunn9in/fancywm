"""PSS clone VA inventory, repeat epochs and full startup graph cleanup."""
...
def observations(rr,files,pid):
    ...
    for label,c,r in zip(LABELS,created,released):
        ...
        first=p.regions(data['clone-va-0.bin'],pid,clone,0);second=p.regions(data['clone-va-1.bin'],pid,clone,0);assert first==second
        for step in range(2):
        ...
        result.append(dict(Label=label,ClonePid=clone,PrivateCommittedBytes=sum(v[2] for v in committed),PrivateReservedBytes=sum(v[2] for v in reserve),PrivateAllocations=len({v[1] for v in committed+reserve}),SourceMapPrivateCommittedBytes=sum(v[2] for v in source if v[4]==0x1000 and v[6]==0x20000),CloneRegions=len(first),SourceRegions=len(source),CloneAbsentBeforeNextWorkload=True))
...
def main():
    ...
    for name in ['Verify-PssGraph.py','Verify-PssHeapControl.py','Verify-FullGraphLifetime.py','Finalize-NativeHeapEvidence.py']:shutil.copyfile(Path(__file__).with_name(name),dest/name)
