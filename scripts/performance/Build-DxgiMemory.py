...
def main():
    ...
    for f in [Path(__file__),*sorted((Path(__file__).parent/'dxgi-memory').glob('*')),*[Path(__file__).parent/'d3d-lifetime'/n for n in ['Resource9.h','Device9.h']]]:
    ...
    for config,opt in [('Debug','/Od'),('Release','/O2')]:
        ...
        for source in sorted((dest/'source').glob('*.cpp')):
            flags='/LD' if source.stem=='DxgiMemory' else ''
            extension='.dll' if source.stem=='DxgiMemory' else '.exe'
