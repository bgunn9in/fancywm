...
$reverificationPath = Join-Path $validationRoot 'contains-window-measurement-reverification.json'
...
Require ($CandidateSnapshotId -ceq 'FWM-CONTAINS-WINDOW-SCAN-20260909-C1R2' -and
    $BaselineSnapshotId -ceq 'FWM-CONTAINS-WINDOW-SCAN-20260909-R0R2' -and
    $ExperimentId -ceq 'EXP-MASTER-SATELLITE-008-CONTAINS-WINDOW-C1R2') 'Delivery must verify the accepted ContainsWindow R0R2/C1R2 experiment.'
$pins = [ordered]@{}
...
Require (($baselineDifferences -join '|') -ceq 'FancyWM/AlgorithmicLayouts/MasterSatelliteLayoutEngine.cs') 'R0/C1 difference exceeds the ordered membership scan.'
$deliveryAllowed = @('PERFORMANCE_STATUS.md', 'PERFORMANCE_TODO.md', 'docs/performance/AUDIT.md',
    ...
    'scripts/performance/README.md', 'scripts/performance/Verify-ContainsWindowDelivery.ps1')
...
Require ($c2.Map['scripts/performance/Verify-ContainsWindowDelivery.ps1'] -ceq $writerHash) 'Delivery verifier is not the frozen C2 source.'
$functionalPins = @{
...
$strictPath = Join-Path $candidateRoot 'validation/contains-window-strict-verification.json'
[void](Pin $strictPath '40F447DF01D7EE376043599ECAB578C54FA9B25DCEE2756216FF2CA30B646DCE')
$strict = Read-Json $strictPath
Require ($strict.Verdict -ceq 'ACCEPT' -and $strict.Accepted -is [bool] -and $strict.Accepted -and
    $strict.Scenario -ceq 'ContainsWindow' -and $strict.PerfId -ceq 'PERF-008' -and
    ...
    $strict.MeasurementRows -eq 840 -and $strict.RawCounters -eq 840 -and $strict.ProcessCount -eq 10 -and
    $strict.AllocationPairs -eq 60 -and $strict.Allocation.Count -eq 60 -and $strict.PairedSemanticComparisons -eq 240 -and
    $strict.BaselineRegressionLeaves -eq 19 -and $strict.BaselineSemanticTestsPassed -eq 18 -and
    $strict.BaselineCapturedPredicateAllocationTestsFailed -eq 1 -and
    $strict.TestFqn -ceq 'FancyWM.Tests.AlgorithmicLayouts.MasterSatelliteContainsWindowTest.ContainsWindowCounterScenario' -and
    ($strict.SourceManifestDifferences -join '|') -ceq 'FancyWM/AlgorithmicLayouts/MasterSatelliteLayoutEngine.cs' -and
    ...
    $strict.TimingClassification -ceq 'mixed' -and $strict.PairedTimingWins -eq 54 -and
    $strict.PairedTimingLosses -eq 6 -and $strict.PairedTimingTies -eq 0 -and
...
foreach ($allocation in $strict.Allocation) {
    $master = $allocation.Case -cmatch '^master-(1|4|9)$'
    Require ($master -or $allocation.Case -cmatch '^(first|last|absent)-(1|4|9)$') 'Unknown accepted allocation case.'
    $baselineBytes = if ($master) { 2400000L } else { 12800000L }
    $candidateBytes = if ($master) { 0L } else { 4000000L }
    Require ($allocation.BaselineBytes -eq $baselineBytes -and $allocation.CandidateBytes -eq $candidateBytes -and
        $allocation.SavedBytes -eq $baselineBytes - $candidateBytes) 'Accepted allocation values differ.'
}
foreach ($entry in @(@('measurements.csv', 'MeasurementsSHA256'), @('contains-window-binary-checks.csv', 'BinaryChecksSHA256'), @('contains-window-process-times.csv', 'ProcessTimesSHA256'))) {
...
$measurementVerifier = Join-Path $c2.Root 'source/scripts/performance/Verify-ContainsWindowScan.ps1'
...
[void](Pin $appendPath '35D1FA1AC9FCE5A67055448991F5CD8558B27691F79D0C35A67A382CBC3753B2')
$append = Read-Json $appendPath
$prefixLength = 37859913L
$prefixHash = '7071B6EA3B83875DD316A66C63700BF734B1AECE814D981FCC5B6A58A08BC919'
Require ($append.Verdict -ceq 'ACCEPT' -and $append.PerfId -ceq 'PERF-008' -and $append.SnapshotId -ceq $CandidateSnapshotId -and $append.ExperimentId -ceq $ExperimentId -and
    ...
    $append.OldSHA256 -ceq $prefixHash -and $append.PrefixSHA256 -ceq $prefixHash -and $append.OldRows -eq 37050 -and $append.OldLines -eq 37051 -and
    $append.AppendedRows -eq 840 -and $append.NewRows -eq 37890 -and $append.NewLines -eq 37891 -and
    $append.NewLength -eq 38906083 -and $append.NewSHA256 -ceq 'E858BC389131CFB4852B829B847ED51B2B4C03B8426DB662CAFBD550550392C8' -and
    $append.SuffixLength -eq 1046170 -and $append.SuffixSHA256 -ceq '3C265AB5BAEB08DC3789DD3E3F5748EDC4E5BC7BAF036B0E3168A48A0AE9AAA9' -and
...
[void](Pin (Join-Path $repositoryRoot 'scripts/performance/Append-ContainsWindowMeasurements.ps1') $append.AppenderSHA256)
...
Require ([regex]::Matches($ledgerText, '\n').Count -eq 37891 -and
    [regex]::Matches($ledgerText, '(?m)^"' + [regex]::Escape($ExperimentId) + '",').Count -eq 840) 'Ledger row counts differ.'
...
$tests = @(foreach ($configuration in @('Debug', 'Release')) {
    foreach ($project in @('FancyWM.Tests', 'FancyWM.Layouts.Tests', 'FancyWM.ThemeEngine.Tests')) {
        $expected = switch ($project) { 'FancyWM.Tests' { if ($configuration -ceq 'Debug') { 1992 } else { 2048 } } 'FancyWM.Layouts.Tests' { 160 } 'FancyWM.ThemeEngine.Tests' { 31 } }
        $expectedRaw = switch ($project) { 'FancyWM.Tests' { if ($configuration -ceq 'Debug') { 2241 } else { 2308 } } 'FancyWM.Layouts.Tests' { 182 } 'FancyWM.ThemeEngine.Tests' { 31 } }
...
$priorRoot = Join-Path $performanceRoot 'FWM-LAYOUT-CALLBACK-CACHE-20260908-C2R3'
$priorSummaryPath = Join-Path $priorRoot 'validation/validation-summary.json'
[void](Pin $priorSummaryPath 'FB146FB42DBC62067C3D3D7B8A54778B743C2E351646770DF196D54FB0F68175')
...
$newLeaves = @(
    ...
    'ContainsWindowChecksSatellitesInOrderAndStopsAtFirstMatch (0)',
    'ContainsWindowChecksSatellitesInOrderAndStopsAtFirstMatch (1)',
    'ContainsWindowChecksSatellitesInOrderAndStopsAtFirstMatch (2)',
    'ContainsWindowChecksSatellitesInOrderAndStopsAtFirstMatch (-1)',
    'ContainsWindowHandlesMissingMasterAndEmptySatellites (False)',
    'ContainsWindowHandlesMissingMasterAndEmptySatellites (True)',
    'ContainsWindowPropagatesExactEqualityExceptionWithoutLaterCallbacks (0)',
    'ContainsWindowPropagatesExactEqualityExceptionWithoutLaterCallbacks (1)',
    'ContainsWindowPropagatesExactEqualityExceptionWithoutLaterCallbacks (3)',
    'ContainsWindowSeesMasterCallbackMutationBeforeSatelliteEnumeration',
    'ContainsWindowMutationThenFalseThrowsOnNextMoveNext (clear)',
    'ContainsWindowMutationThenFalseThrowsOnNextMoveNext (replace)',
    'ContainsWindowMutationThenTrueShortCircuitsBeforeNextMoveNext (clear)',
    'ContainsWindowMutationThenTrueShortCircuitsBeforeNextMoveNext (replace)',
    'ContainsWindowPreservesRuntimeStateAndReadOnlyViewIdentity (False)',
    'ContainsWindowPreservesRuntimeStateAndReadOnlyViewIdentity (True)',
    'ContainsWindowAvoidsCapturedPredicateAllocation',
    'ContainsWindowCounterScenario'
)
$affectedPins = @{
    Debug = @{ Leaves = 358; Raw = 401; Trx = '30660DE33550B7B3AA028D83BE751950AAE58861EB445682C586070B04DD1986'; Log = '93F84969F520FE5C7023B3C3CFC4F8CE11D548AED23039CD18858FF385AB98DE' }
    Release = @{ Leaves = 382; Raw = 432; Trx = 'EB2D54EE83A12B09CF35360A98998A4EBB9BDB5F0DD51434ACE39D22EE6080D3'; Log = 'D6528F3A3C01422C2B1BE75CB2F1766754D4E190FF474B6E1199A194249AD4E8' }
}
$newKeysByConfiguration = @{}
$candidateTests = @(foreach ($configuration in @('Debug', 'Release')) {
    $targeted = Read-PassedTrx (Join-Path $candidateRoot "validation/$configuration-contains-window-targeted.trx") 19
    $acceptedRegression = @($strict.CandidateRegressions | Where-Object Configuration -CEQ $configuration)
    Require ($acceptedRegression.Count -eq 1 -and $targeted.Raw -eq 25 -and
    ...
    $targeted
    $expectedAffected = $affectedPins[$configuration]
    $affected = Read-PassedTrx (Join-Path $candidateRoot "validation/$configuration-contains-window-affected.trx") $expectedAffected.Leaves
    Require ($affected.Raw -eq $expectedAffected.Raw -and $affected.TrxSHA256 -ceq $expectedAffected.Trx -and
        $affected.LogSHA256 -ceq $expectedAffected.Log) "Affected suite differs from accepted C1: $configuration"
    foreach ($key in $targeted.Keys) { Require ($key -cin $affected.Keys) "Affected regression is missing: $configuration/$key" }
    $affected
})
...
$summary = [ordered]@{
    Verdict = 'ACCEPT'; ReportVersion = 1; SnapshotId = $SnapshotId; PerfId = 'PERF-008'
    BoundedStage = 'MasterSatelliteLayoutEngine.ContainsWindow ordered enumeration without captured predicate'
    ...
    C1MeasurementAcceptance = [ordered]@{
        ...
        ProcessCount = 10; MeasurementRows = 840; RawCounters = 840; AllocationPairs = 60; SemanticComparisons = 240
        BaselineRegressionLeaves = 19; BaselineSemanticTestsPassed = 18; BaselineCapturedPredicateAllocationTestsFailed = 1
    ...
    OptimizationLedger = [ordered]@{
        Rows = 37890; Bytes = $ledger.Length; SHA256 = Hash $ledgerPath; PreviousRows = 37050; PreviousBytes = $prefixLength; PreviousSHA256 = $prefixHash
        AppendedRows = 840; AppendedBytes = $suffixLength; AppendedSHA256 = $append.SuffixSHA256; ExactHistoricalPrefix = $true; ExactAcceptedSuffix = $true; AppendProofSHA256 = Hash $appendPath
    }
    ...
    Limitations = 'Managed ContainsWindow membership calls with resident-first equality, callback order and the same read-only collection enumeration. No broad transaction or clone reuse. Timing is mixed and descriptive; no general speedup claim. PERF-008 and PERF-010 remain IN_PROGRESS. Native COM/WPF/DPI behavior, transition scheduling/call fan-out, presentation latency, process CPU/GPU and energy remain E2E_PENDING / NOT_MEASURED.'
