...
function Test-FocusRejectedRegressionTrx([string]$Path, [bool]$ExpectedRed, [string]$ExpectedAssemblyPath = '') {
    $expectedCases = [ordered]@{
        'RejectedFallbackAdmissionAllocatesNoRequest' = @('superseded','expired','stopped')
        'FocusRejectedAdmissionCounterScenario' = @('')
    }
    $expectedClass = 'FancyWM.Tests.Utilities.FocusHelperTest'
    $regressionName = 'RejectedFallbackAdmissionAllocatesNoRequest'
    ...
    Assert-Condition ($all.Count -eq 5 -and $leaves.Count -eq 4 -and $topResults.Count -eq 2 -and $definitions.Count -eq 2 -and $entries.Count -eq 2) "Regression raw/leaf/definition/entry dimensions differ: $Path"
    ...
    foreach ($result in $topResults) {
        ...
        if ($dataDriven) {
            $aggregateOutcome = if ($ExpectedRed) { 'Failed' } else { 'Passed' }
    ...
    foreach ($leaf in $leaves) {
        ...
        if ($mustFail) {
            ...
            $modeMatch = [regex]::Match($leaf.testName, '^RejectedFallbackAdmissionAllocatesNoRequest \((superseded|expired|stopped)\)$')
            Assert-Condition ($modeMatch.Success) 'Unknown rejected-admission allocation case.'
            $failurePattern = '^Assert\.AreEqual failed\. Expected:<0>\. Actual:<(?<allocated>[1-9][0-9]*)>\. Rejected fallback admission still allocates a request: mode=' +
                [regex]::Escape($modeMatch.Groups[1].Value) + ', total=\k<allocated> B, per-call=(?<perCall>[0-9]+[.,][0-9]{3}) B\.$'
            $failureMatch = [regex]::Match($messages[0].InnerText.TrimEnd(), $failurePattern)
            Assert-Condition ($failureMatch.Success) 'Baseline failed for a reason other than rejected-admission allocation.'
            $allocated = 0L
            Assert-Condition ([long]::TryParse($failureMatch.Groups['allocated'].Value,
                [Globalization.NumberStyles]::None, [Globalization.CultureInfo]::InvariantCulture, [ref]$allocated) -and $allocated -gt 0) 'Invalid baseline allocation failure value.'
            $expectedPerCall = ([double]$allocated / 100000).ToString('F3', [Globalization.CultureInfo]::InvariantCulture)
            Assert-Condition ($failureMatch.Groups['perCall'].Value.Replace(',', '.') -ceq $expectedPerCall) 'Baseline allocation failure total/per-call values differ.'
            $failures++
        }
    ...
    foreach ($name in @('total','executed','passed','failed','error','timeout','aborted','inconclusive','passedButRunAborted','notRunnable','notExecuted','disconnected','warning','completed','inProgress','pending')) {
        $expected = switch ($name) {
            { $_ -cin @('total','executed') } { 5 }
            'passed' { 5 - $expectedRawFailures }
...
$testMethod = 'FocusRejectedAdmissionCounterScenario'
$testFqn = "$testClass.$testMethod"
$cases = @('superseded','expired','stopped')
$calls = 100000L
$units = [ordered]@{
...
$requiredSources = @{
    ...
    RegressionSource = 'FancyWM.Tests/Utilities/FocusHelperTest.RejectedAdmission.cs'
    FixtureSource = 'FancyWM.Tests/Utilities/FocusHelperTest.RejectedAdmission.cs'
    RunnerSource = 'scripts/performance/Measure-MicaRefresh.ps1'
    ArchiveScriptSource = 'scripts/performance/Archive-FocusRejectedBaseline.ps1'
    VerifierSource = 'scripts/performance/Verify-FocusRejectedAdmission.ps1'
    AppendScriptSource = 'scripts/performance/Append-FocusRejectedMeasurements.ps1'
}
foreach ($path in @($requiredSources.Values) + @('scripts/performance/Verify-FocusRejectedAdmission.ps1')) {
...
$methodPattern = '(?ms)^            internal Task<bool> Enqueue\(object token, IntPtr window, INative native\)\r?\n.*?(?=^            private void RunWorker)'
$oldMethods = [regex]::Matches($oldSource, $methodPattern)
$newMethods = [regex]::Matches($newSource, $methodPattern)
Assert-Condition ($oldMethods.Count -eq 1 -and $newMethods.Count -eq 1) 'Enqueue method boundary differs.'
$expectedOld = @'
            internal Task<bool> Enqueue(object token, IntPtr window, INative native)
            {
                var request = new FallbackRequest(token, window, native);
                Thread? start = null;
                lock (m_gate)
                {
                    ...
                    if (m_worker == null)
                    {
                        start = m_worker = new Thread(RunWorker)
                        {
                            IsBackground = true,
...
'@
Assert-Condition ($oldMethods[0].Value.Replace([string][char]13, '').TrimEnd() -ceq $expectedOld.Replace([string][char]13, '').TrimEnd()) 'Baseline request allocation before admission differs.'
Assert-Condition ($newMethods[0].Value.Replace([string][char]13, '').TrimEnd() -ceq $expectedNew.Replace([string][char]13, '').TrimEnd()) 'Candidate request allocation after admission differs.'
...
Assert-Condition ($reconstructedBaseline.Replace([string][char]13, '') -ceq $oldSource.Replace([string][char]13, '')) 'Source delta exceeds deferring rejected Enqueue request allocation.'

$fixtureText = Get-Content -LiteralPath (Join-Path $candidateRoot 'source/FancyWM.Tests/Utilities/FocusHelperTest.RejectedAdmission.cs') -Raw
Assert-Condition ($fixtureText.Contains('private const int RejectedAdmissionWarmupCalls = 1_000;') -and $fixtureText.Contains('private const int RejectedAdmissionMeasuredCalls = 100_000;') -and
    $fixtureText.Contains('value.ToString(CultureInfo.InvariantCulture)')) 'Common fixture iteration/counter protocol differs.'
Assert-Condition ((Get-Sha256 $PSCommandPath) -ceq $candidate.ByPath['scripts/performance/Verify-FocusRejectedAdmission.ps1'].SHA256) 'Verifier differs from the common frozen protocol.'
...
Assert-Condition ($provenance.Snapshot -ceq $BaselineSnapshotId -and
    ...
    $provenance.SemanticTestsPassed -eq 1 -and $provenance.RejectedAdmissionAllocationTestsFailed -eq 3) `
...
$redMethods = @('RejectedFallbackAdmissionAllocatesNoRequest','FocusRejectedAdmissionCounterScenario')
...
$redResult = Test-FocusRejectedRegressionTrx (Get-SafeRelativePath $baselineRoot $provenance.RedTrx) $true (Join-Path $repositoryRoot "FancyWM.Tests/bin/Release/$targetFramework/FancyWM.Tests.dll")
$redPassed = $redResult.Passed
$redFailed = $redResult.Failed
$candidateRegressions = @(foreach ($configuration in @('Debug','Release')) {
    $trxPath = Join-Path $candidateRoot "validation/$configuration-focus-rejected-targeted.trx"
    $logPath = [IO.Path]::ChangeExtension($trxPath, '.log')
    $result = Test-FocusRejectedRegressionTrx $trxPath $false (Join-Path $repositoryRoot "FancyWM.Tests/bin/$configuration/$targetFramework/FancyWM.Tests.dll")
...
$binaryChecksPath = Join-Path $experimentRoot 'focus-rejected-binary-checks.csv'
...
$processes = @()
$counterPattern = '^PERFCOUNTER focus-rejected-(superseded|expired|stopped) (' + ($metrics -join '|') + ') (0|[1-9][0-9]*)$'
foreach ($runName in $expectedRunOrder) {
    ...
    Assert-Condition ($rawLines.Count -eq 27) "Raw Console counter envelope differs: $runName"
    $counterLines = $rawLines
    Assert-Condition ($counterLines.Count -eq 27) "Raw counter line count differs: $runName"
...
Assert-Condition ($rawCounters.Count -eq 270) 'Raw counter matrix is incomplete.'
...
Assert-Condition ($rows.Count -eq 270) "Measurement row count differs: $($rows.Count)"
...
foreach ($row in $rows) {
    $scenario = [regex]::Match($row.scenario,
        '^Actual FocusHelper\.RequestSequence\.Enqueue; focus-rejected-(superseded|expired|stopped);1000 discarded warmups then100000 calls$')
    ...
    Assert-Condition ($row.variant -cin @('baseline', 'candidate') -and
        ...
        $row.iterations -ceq '100000' -and $row.perf_id -ceq 'PERF-030' -and
    ...
    $expected = switch -CaseSensitive ($row.metric) {
        ...
        { $_ -cin @('calls','rejected','current-preserved') } { $calls }
...
foreach ($caseName in $cases) {
    ...
    foreach ($pair in 1..5) {
        ...
        Assert-Condition ($candidateBytes -eq 0 -and $candidateBytes -lt $baselineBytes) "Candidate allocation was not eliminated: $pair/$caseName"
...
$processTimesPath = Join-Path $experimentRoot 'focus-rejected-process-times.csv'
...
$report = [ordered]@{
    ...
    Scenario = 'FocusRejectedAdmission'; PerfId = 'PERF-030'
    ...
    ExactSourceReplacement = 'Enqueue constructs FallbackRequest only after the existing current-token check under the same gate, before pending completion'
    BaselineArchiveFiles = $archiveFiles.Count; BaselineArchiveBytes = $archiveBytes
    BaselineSemanticTestsPassed = $redPassed; BaselineRejectedAdmissionAllocationTestsFailed = $redFailed
    ...
    Limitations = 'Managed rejection of pre-created superseded, expired and stopped focus requests. The measurement includes Enqueue and common completion/current-state accounting. Admitted fallback work, lock contention, native focus/input/COM/WPF/DPI, timeout or late-focus effects, WPR, whole-process CPU/GPU, presentation latency and energy remain E2E_PENDING / NOT_MEASURED. Timing is descriptive.'
}
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = Join-Path $candidateRoot 'validation/focus-rejected-strict-verification.json'
}
