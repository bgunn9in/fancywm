...
$reverificationPath = Join-Path $validationRoot 'focus-rejected-measurement-reverification.json'
...
Require ($CandidateSnapshotId -ceq 'FWM-FOCUS-REJECTED-ADMISSION-20260909-C1' -and
    $BaselineSnapshotId -ceq 'FWM-FOCUS-REJECTED-ADMISSION-20260909-R0' -and
    $ExperimentId -ceq 'EXP-FOCUS-030-REJECTED-ADMISSION-C1') 'Delivery must verify the accepted FocusRejectedAdmission R0/C1 experiment.'
$pins = [ordered]@{}
...
Require (($baselineDifferences -join '|') -ceq 'FancyWM/Utilities/FocusHelper.cs') 'R0/C1 difference exceeds FocusHelper rejected-admission allocation placement.'
$deliveryAllowed = @('PERFORMANCE_STATUS.md', 'PERFORMANCE_TODO.md', 'docs/performance/AUDIT.md',
    ...
    'scripts/performance/README.md', 'scripts/performance/Verify-FocusRejectedDelivery.ps1')
...
Require ($c2.Map['scripts/performance/Verify-FocusRejectedDelivery.ps1'] -ceq $writerHash) 'Delivery verifier is not the frozen C2 source.'
$functionalPins = @{
...
$strictPath = Join-Path $candidateRoot 'validation/focus-rejected-strict-verification.json'
[void](Pin $strictPath)
$strict = Read-Json $strictPath
Require ($strict.Verdict -ceq 'ACCEPT' -and $strict.Accepted -is [bool] -and $strict.Accepted -and
    $strict.Scenario -ceq 'FocusRejectedAdmission' -and $strict.PerfId -ceq 'PERF-030' -and
    ...
    $strict.MeasurementRows -eq 270 -and $strict.RawCounters -eq 270 -and $strict.ProcessCount -eq 10 -and
    $strict.AllocationPairs -eq 15 -and $strict.Allocation.Count -eq 15 -and $strict.PairedSemanticComparisons -eq 90 -and
    $strict.BaselineRegressionLeaves -eq 4 -and $strict.BaselineSemanticTestsPassed -eq 1 -and
    $strict.BaselineRejectedAdmissionAllocationTestsFailed -eq 3 -and
    $strict.TestFqn -ceq 'FancyWM.Tests.Utilities.FocusHelperTest.FocusRejectedAdmissionCounterScenario' -and
...
foreach ($allocation in $strict.Allocation) {
    Require ($allocation.Case -cin @('superseded','expired','stopped') -and
...
foreach ($entry in @(@('measurements.csv', 'MeasurementsSHA256'), @('focus-rejected-binary-checks.csv', 'BinaryChecksSHA256'), @('focus-rejected-process-times.csv', 'ProcessTimesSHA256'))) {
...
$measurementVerifier = Join-Path $c2.Root 'source/scripts/performance/Verify-FocusRejectedAdmission.ps1'
...
$prefixLength = 38906083L
$prefixHash = 'E858BC389131CFB4852B829B847ED51B2B4C03B8426DB662CAFBD550550392C8'
Require ($append.Verdict -ceq 'ACCEPT' -and $append.PerfId -ceq 'PERF-030' -and $append.SnapshotId -ceq $CandidateSnapshotId -and $append.ExperimentId -ceq $ExperimentId -and
    ...
    $append.OldSHA256 -ceq $prefixHash -and $append.PrefixSHA256 -ceq $prefixHash -and $append.OldRows -eq 37890 -and $append.OldLines -eq 37891 -and
    $append.AppendedRows -eq 270 -and $append.NewRows -eq 38160 -and $append.NewLines -eq 38161 -and
    $append.NewLength -eq $prefixLength + $append.SuffixLength -and $append.NewSHA256 -cmatch '^[0-9A-F]{64}$' -and
    $append.SuffixLength -gt 0 -and $append.SuffixSHA256 -cmatch '^[0-9A-F]{64}$' -and
...
[void](Pin (Join-Path $repositoryRoot 'scripts/performance/Append-FocusRejectedMeasurements.ps1') $append.AppenderSHA256)
...
Require ([regex]::Matches($ledgerText, '\n').Count -eq 38161 -and
    [regex]::Matches($ledgerText, '(?m)^"' + [regex]::Escape($ExperimentId) + '",').Count -eq 270) 'Ledger row counts differ.'
...
$tests = @(foreach ($configuration in @('Debug', 'Release')) {
    foreach ($project in @('FancyWM.Tests', 'FancyWM.Layouts.Tests', 'FancyWM.ThemeEngine.Tests')) {
        $expected = switch ($project) { 'FancyWM.Tests' { if ($configuration -ceq 'Debug') { 1996 } else { 2052 } } 'FancyWM.Layouts.Tests' { 160 } 'FancyWM.ThemeEngine.Tests' { 31 } }
        $expectedRaw = switch ($project) { 'FancyWM.Tests' { if ($configuration -ceq 'Debug') { 2246 } else { 2313 } } 'FancyWM.Layouts.Tests' { 182 } 'FancyWM.ThemeEngine.Tests' { 31 } }
...
$priorRoot = Join-Path $performanceRoot 'FWM-CONTAINS-WINDOW-SCAN-20260909-C2R1'
$priorSummaryPath = Join-Path $priorRoot 'validation/validation-summary.json'
[void](Pin $priorSummaryPath '3355330A5A4AE463BD61554CC5DFC6458C6CA55D257EA7ECA899CD190281DC79')
$prior = Read-Json $priorSummaryPath
Require ($prior.Verdict -ceq 'ACCEPT') 'Prior accepted gate changed.'
$newLeaves = @(
    'RejectedFallbackAdmissionAllocatesNoRequest (superseded)',
    'RejectedFallbackAdmissionAllocatesNoRequest (expired)',
    'RejectedFallbackAdmissionAllocatesNoRequest (stopped)',
    'FocusRejectedAdmissionCounterScenario'
)
$affectedFragments = @('FocusHelperTest', 'MainWindowLifetimeTest', 'WindowDraggerTest',
    'ModifierWindowMoverTest', 'TilingServiceAlgorithmicIntegrationTest', 'MultiDisplayTilingServiceIntegrationTest')
$affectedFilter = ($affectedFragments | ForEach-Object { "FullyQualifiedName~$_" }) -join '|'
$targetedFilter = 'FullyQualifiedName=FancyWM.Tests.Utilities.FocusHelperTest.RejectedFallbackAdmissionAllocatesNoRequest|FullyQualifiedName=FancyWM.Tests.Utilities.FocusHelperTest.FocusRejectedAdmissionCounterScenario'

$newKeysByConfiguration = @{}
$candidateTests = @(foreach ($configuration in @('Debug', 'Release')) {
    $targeted = Read-PassedTrx (Join-Path $candidateRoot "validation/$configuration-focus-rejected-targeted.trx") 4
    $acceptedRegression = @($strict.CandidateRegressions | Where-Object Configuration -CEQ $configuration)
    Require ($acceptedRegression.Count -eq 1 -and $targeted.Raw -eq 5 -and
    ...
    $affected = Read-PassedTrx (Join-Path $candidateRoot "validation/$configuration-focus-rejected-affected.trx") $expectedAffectedKeys.Count
    $affectedKeys = [string[]]@($affected.Keys)
    Require ($affected.Raw -eq $previousAffectedResults.Count + $targeted.Raw -and
        $expectedAffectedKeys.SetEquals($affectedKeys)) "Affected test composition differs from the declared prior union plus four new leaves: $configuration"
...
$summary = [ordered]@{
    ...
    BoundedStage = 'FocusHelper.RequestSequence.Enqueue skips request allocation after rejected admission'
    ...
    C1MeasurementAcceptance = [ordered]@{
        ...
        ProcessCount = 10; MeasurementRows = 270; RawCounters = 270; AllocationPairs = 15; SemanticComparisons = 90
        BaselineRegressionLeaves = 4; BaselineSemanticTestsPassed = 1; BaselineRejectedAdmissionAllocationTestsFailed = 3
    ...
    OptimizationLedger = [ordered]@{
        Rows = 38160; Bytes = $ledger.Length; SHA256 = Hash $ledgerPath; PreviousRows = 37890; PreviousBytes = $prefixLength; PreviousSHA256 = $prefixHash
        AppendedRows = 270; AppendedBytes = $suffixLength; AppendedSHA256 = $append.SuffixSHA256; ExactHistoricalPrefix = $true; ExactAcceptedSuffix = $true; AppendProofSHA256 = Hash $appendPath
    }
    ...
    Limitations = 'Managed rejected FocusHelper.RequestSequence.Enqueue admission for superseded, expired and stopped tokens. Existing native activation, caller wait, callback ordering, worker ownership and cleanup remain unchanged. Timing is descriptive; allocation savings do not establish native latency or whole-process speedup. PERF-030 and PERF-010 remain IN_PROGRESS. Initial synchronous native activation bounds, late native focus, native thread/handle retention, WPF/DPI behavior, transition scheduling/fan-out, process CPU/GPU and energy remain E2E_PENDING / NOT_MEASURED.'
