...
def main():
    ...
    prior=s.ART/'FWM-PERF016-D3D-20260913-R1';assert s.sha(prior/'verification.json')=='5FF53FF4A0E2273ABD818C7DFA83A1124B7FFFC589E88039DFC611262DB55B1F';assert s.sha(prior/'evidence-manifest.json')=='90C4458AAA6B0FE8DA2CD11EDA098ED6B0B453310691E06723B3DFB782DEB4D3';rows=s.read(prior/'evidence-manifest.json')
    ...
    for name in ['D3D_NATIVE_OWNERS.md','PSS_NATIVE_MEMORY.md']:
    ...
    s.write(dest/'verification.json',dict(Verdict='D3D_MAPPING_ENTRY_VERIFIED',PriorFiles=len(rows),PriorBytes=sum(r['Bytes'] for r in rows),PriorManifestSHA256=s.sha(prior/'evidence-manifest.json'),EntryReviewSHA256=s.sha(dest/'entry-review.json'),EtlPathsAndSizesUnchanged=True,EtlFiles=32,FullEtlHashPassRepeated=False,NewTracingSession=False,ProductionChanged=False,PerformanceClaim=False,StageAccepted=False,LedgerAppended=False));print(s.sha(dest/'verification.json'),flush=True)
