...
def main():
    ...
    for f in [Path(__file__),*sorted((Path(__file__).parent/'d3d-map').glob('*')),*[Path(__file__).parent/'d3d-lifetime'/n for n in ['Resource9.h','Device9.h']]]:
    ...
    for config,opt in [('Debug','/Od'),('Release','/O2')]:
        ...
        for source in sorted((dest/'source').glob('*.cpp')):body+=f'cl /nologo /std:c++17 /EHsc /W4 /WX /Zi {opt} /MT "{source}" /link /OUT:"{out / (source.stem+".exe")}" /PDB:"{out / (source.stem+".pdb")}"\nif errorlevel 1 exit /b %errorlevel%\n'
