...
Assert-Condition ([StringComparer]::OrdinalIgnoreCase.Equals(
    [IO.Path]::GetFullPath((Get-Location).Path), $repositoryRoot)) 'Run the archive script from the repository root.'
...
Assert-Condition ($LASTEXITCODE -eq 0 -and $head -ceq $snapshot.Commit -and
    $head -ceq '4fb943be3ee3768bb6de42b16b0a22bdbafa2e0b') 'Baseline/current HEAD differs.'
$tree = (& git -C $repositoryRoot rev-parse 'HEAD^{tree}').Trim()
Assert-Condition ($LASTEXITCODE -eq 0 -and $tree -ceq $snapshot.Tree) 'Baseline/current tree differs.'
...
Assert-Condition ($LASTEXITCODE -eq 0 -and @($submodules | Where-Object { $_ -cmatch '^[+-U]' }).Count -eq 0 -and
    ($submodules -join '|') -ceq ($snapshot.Submodules -join '|')) 'Baseline/current submodule identities differ.'
...
$frozenFiles = @(Get-ChildItem -LiteralPath (Join-Path $snapshotRoot 'source') -Recurse -File -Force)
Assert-Condition ($frozenFiles.Count -eq $manifest.Count) 'Frozen source contains missing or unlisted files.'
foreach ($file in $frozenFiles) {
    $relative = [IO.Path]::GetRelativePath((Join-Path $snapshotRoot 'source'), $file.FullName).Replace('\', '/')
    Assert-Condition ($sourcePaths.Contains($relative)) "Unlisted frozen source: $relative"
}
...
'@
$methodMatches = [regex]::Matches($productionText, '(?ms)^            internal Task<bool> Enqueue\(object token, IntPtr window, INative native\)\r?\n.*?(?=^            private void RunWorker)')
Assert-Condition ($methodMatches.Count -eq 1 -and
    $methodMatches[0].Value.Replace([string][char]13, '').TrimEnd() -ceq
    $oldMethod.Replace([string][char]13, '').TrimEnd()) 'Baseline must contain the original Enqueue allocation before the locked admission check.'

foreach ($relativeOutput in @('release-test-binaries','release-test-binaries.csv','release-test-binaries.provenance.json','validation/baseline-release-build.log','validation/baseline-focus-rejected-red.trx','validation/baseline-focus-rejected-red.log')) {
...
$redTrxName = 'baseline-focus-rejected-red.trx'
$redTrx = Join-Path $validationRoot $redTrxName
$redLog = Join-Path $validationRoot 'baseline-focus-rejected-red.log'
...
Assert-Condition ($redExitCode -eq 1) "Baseline rejected-admission allocation regressions did not fail exactly as expected: $redExitCode"
Assert-Condition (Test-Path -LiteralPath $redTrx -PathType Leaf) 'Baseline red TRX missing'

$expectedAssembly = Join-Path $repositoryRoot 'FancyWM.Tests/bin/Release/net10.0-windows10.0.18362.0/FancyWM.Tests.dll'
$redResult = Test-FocusRejectedRegressionTrx $redTrx $true $expectedAssembly
foreach ($record in $buildInputs) {
...
$provenance = [ordered]@{
    ...
    Scope = 'PERF-030 FocusHelper.RequestSequence.Enqueue avoids request allocation for superseded, expired and stopped admission'
    ...
    RedLog = 'validation/baseline-focus-rejected-red.log'
    RedLogSHA256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $redLog).Hash
    SemanticTestsPassed = $redResult.Passed
    RejectedAdmissionAllocationTestsFailed = 3
    RedRegressionLeaves = $redResult.Leaves
    RedRegressionRawResults = $redResult.RawResults
