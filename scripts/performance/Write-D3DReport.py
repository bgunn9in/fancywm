"""Publish only the newly verified scoped observations into the live documents."""
...
def proof(name):return s.ART/(PREFIX+name)/'verification.json'
def main():
    graph=s.read(proof('D4'));pss=s.read(proof('D3'));cal=s.read(proof('V3'));assert graph['Verdict']=='SCOPED_D3D9_GRAPH_PRIVATE_OWNER_OBSERVATIONS_VERIFIED' and graph['PssProofSHA256']==s.sha(proof('D3')) and graph['CalibrationSHA256']==s.sha(proof('V3'))
    assert len(graph['Configurations'])==2 and all(c['FinalActivePrivateOwners']==0 and c['ObserverClosed'] and c['PrivateOwnerCountNoGrowth'] for c in graph['Configurations'])
    assert all(not c['ApiFailures'] and not c['UnownedMapEvents'] and c['UntrackedTextureParentCount']==0 for c in graph['Configurations'])
    assert all(not x['MapsCoveringEntireCapture'] for c in graph['Configurations'] for x in c['SnapshotMapCorrelation'])
    table='\n'.join(f"| {c['Configuration']} | {c['RegisteredPrivateOwners']:,} | {c['PrivateOwnerReleases']:,} | {c['MapUnmapPairs']:,} | {' / '.join(str(x['Active']) for x in c['Cohorts'][:3])} | {c['FinalActivePrivateOwners']} |" for c in graph['Configurations'])
    pss_table='\n'.join(f"| {c['Configuration']} | {' / '.join(str(x['PrivateCommittedBytes']) for x in c['NativeSnapshots'][1:4])} | {c['NativeSnapshots'][-1]['PrivateCommittedBytes']} | {c['PrivateCommittedNoGrowth']} | {c['GuiCriterionPassed']} |" for c in pss['Configurations'])
    hashes='\n'.join(f"- {name}: `{s.sha(proof(name))}`." for name in ['R0','V1','V2','V3','B1','D1','D3','D4'])
    body=f'''# Native D3D ownership observations — 2026-09-13
...
{hashes}
'''
    dest=s.ROOT/'docs/performance/D3D_NATIVE_OWNERS.md';assert not dest.exists();dest.write_text(body,encoding='utf-8',newline='\n')
    for rel,link in [('PERFORMANCE_STATUS.md','docs/performance/D3D_NATIVE_OWNERS.md'),('PERFORMANCE_TODO.md','docs/performance/D3D_NATIVE_OWNERS.md'),('docs/performance/AUDIT.md','D3D_NATIVE_OWNERS.md'),('docs/performance/IMPLEMENTATION_RESULTS.md','D3D_NATIVE_OWNERS.md'),('scripts/performance/README.md','../../docs/performance/D3D_NATIVE_OWNERS.md')]:
        p=s.ROOT/rel;raw=p.read_bytes();pos=raw.index(b'\n')+1;newline=b'\r\n' if raw[:pos].endswith(b'\r\n') else b'\n'
        block=f'''\nLatest verified continuation (2026-09-13): `FWM-PERF016-D3D-20260913-R1` —
[native D3D ownership observations]({link}).
...
'''.replace('\n','\r\n' if newline==b'\r\n' else '\n').encode('utf-8')
        p.write_bytes(raw[:pos]+block+raw[pos:])
